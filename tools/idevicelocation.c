...
int main(int argc, char **argv)
{
	...
	if (service && service->port > 0) {
		...
		if (rerr != SERVICE_E_SUCCESS) {
			printf("ERROR: Could not connect to DTSimulateLocation!\n");
		} else {
			if (stop) {
		...
		printf("ERROR: Could not start DTSimulateLocation service! Remember that you have to mount the Developer disk image on your device if you want to use the DTSimulateLocation service.\n");
	}
